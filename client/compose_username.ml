...
let build_result send_username (textbox_content, set_textbox_content) =
  let submit_and_then_clear =
    Vdom.Effect.Many [ send_username textbox_content; set_textbox_content "" ]
  in
  let on_ret =
    let is_key_ret key =
      String.equal
...
let component ~send_username =
  ...
  in
  return (build_result <$> send_username <*> user_state)
;;
